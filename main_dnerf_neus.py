...
if __name__ == '__main__':
    ...
    parser.add_argument('--canonical_direction', action='store_true')
    parser.add_argument('--not_use_viewdirs', action='store_true')
    ...
    parser.add_argument('--sdf_layers', type=int, default=2)
    parser.add_argument('--color_layers', type=int, default=3)
    parser.add_argument('--encoder', type=str, default='HashGrid')
    ...
    parser.add_argument('--smpl_guided_sample', type=str, default=None)
    parser.add_argument('--white_bg', action='store_true')
    parser.add_argument('--neus_network', action='store_true')
    parser.add_argument('--use_random_bg', action='store_true')
    parser.add_argument('--use_mask_loss', action='store_true')
    parser.add_argument('--sdf_use_pe', action='store_true')
    parser.add_argument('--geometry_init_check', action='store_true')
    parser.add_argument('--geometry_init', action='store_true')
    ...
    if opt.basis:
        ...
        from dnerf.network_neus import NeusNetwork
    ...
    model = NeusNetwork(
        ...
        num_layers=opt.sdf_layers,
        num_layers_color=opt.color_layers
    )
    ...
    if opt.test:
        ...
        if opt.gui:
            ...
            def query_func(pts, t):
                with torch.no_grad():
                    with torch.cuda.amp.autocast(enabled=trainer.fp16):
                        sdf = - trainer.model.sdf(pts.to(trainer.device), t.to(trainer.device))
                return sdf

            trainer.save_mesh(query_func, time = 0, resolution=256, threshold=0)
        ...
        trainer = Trainer('ngp', opt, model, device=device, workspace=opt.workspace, optimizer=optimizer, criterion=criterion, ema_decay=0.95, fp16=opt.fp16, lr_scheduler=scheduler, scheduler_update_every_step=True, metrics=[PSNRMeter()], use_checkpoint=opt.ckpt, eval_interval=opt.eval_interval)
